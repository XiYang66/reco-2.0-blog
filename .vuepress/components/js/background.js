...
function getWebGLContext(e) {
    ...
    var l;
    var u;
    var c = a ? i.HALF_FLOAT : r.HALF_FLOAT_OES;
...
function initFramebuffers() {
    ...
    var r = getResolution(config.DYE_RESOLUTION);
    var t = ext.halfFloatTexType;
    var n = ext.formatRGBA;
    var i = ext.formatRG;
    var a = ext.formatR;
    var o = ext.supportLinearFiltering ? gl.LINEAR : gl.NEAREST;
...
function initBloomFramebuffers() {
    ...
    var r = ext.halfFloatTexType;
    var t = ext.formatRGBA;
    var n = ext.supportLinearFiltering ? gl.LINEAR : gl.NEAREST;
...
window.initBackground = function e() {
    e.loaded ||
        ((e.loaded = !0),
            updateKeywords(),
            initFramebuffers(),
...
window.addEventListener('visibilityChangeEvent', window.initBackground),
    window.addEventListener('DOMContentLoaded', window.initBackground);
...
function calcDeltaTime() {
    var e = Date.now();
    var r = (e - lastUpdateTime) / 1e3;

    return (r = Math.min(r, 0.016666)), (lastUpdateTime = e), r;
}
function resizeCanvas() {
    var e = scaleByPixelRatio(canvas.clientWidth);
    var r = scaleByPixelRatio(canvas.clientHeight);

    return (
        (canvas.width !== e || canvas.height !== r) && ((canvas.width = e), (canvas.height = r), !0)
    );
}
...
function render(e) {
    ...
    var r = e == null ? gl.drawingBufferWidth : e.width;
    var t = e == null ? gl.drawingBufferHeight : e.height;

    gl.viewport(0, 0, r, t);
    var n = e == null ? null : e.fbo;

    config.TRANSPARENT || drawColor(n, normalizeColor(config.BACK_COLOR)),
        e == null && config.TRANSPARENT && drawCheckerboard(n),
        drawDisplay(n, r, t);
}
...
function applyBloom(e, r) {
    if (!(bloomFramebuffers.length < 2)) {
        ...
        var i = config.BLOOM_THRESHOLD - n;
        var a = 2 * n;
        var o = 0.25 / n;
...
function splatPointer(e) {
    ...
    var t = e.deltaY * config.SPLAT_FORCE;
...
window.addEventListener('touchend', function (e) {
    ...
    window.addEventListener('keydown', function (e) {
        e.code === 'KeyP' && (config.PAUSED = !config.PAUSED),
            e.key === ' ' && splatStack.push(parseInt(20 * Math.random()) + 5);
    }),
    document.addEventListener('mousedown', function (e) {
        if (!switchPage || !switchPage.switched) {
            var r = scaleByPixelRatio(e.pageX);
            var t = scaleByPixelRatio(e.pageY);
            var n = pointers.find(function (e) {
                return e.id == -1;
            });

            n == null && (n = new pointerPrototype()),
    ...
    document.addEventListener('touchstart', function (e) {
        if (!switchPage || !switchPage.switched) {
            ...
            for (var t = 0; t < r.length; t++) {
                ...
                var i = scaleByPixelRatio(r[t].pageY);
